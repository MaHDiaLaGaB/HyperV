...
class Event(Base):
    ...
    detected_at: Mapped[dt.datetime] = mapped_column(DateTime(timezone=True), default=dt.datetime.utcnow, nullable=False)
    severity: Mapped[Optional[int]] = mapped_column(Integer)
    description: Mapped[Optional[str]] = mapped_column(Text)

    location: Mapped[Optional[str]] = mapped_column(Geometry("POINT"))
    pipeline_id: Mapped[Optional[int]] = mapped_column(ForeignKey("pipelines.id", ondelete="SET NULL"))
    ...
    pipeline:     Mapped[Optional["Pipeline"]] = relationship(back_populates="events")  # noqa: F821
    asset:        Mapped[Optional["Asset"]] = relationship(back_populates="events")  # noqa: F821
    alerts:       Mapped[List["Alert"]] = relationship(back_populates="event", cascade="all, delete-orphan")  # noqa: F821
